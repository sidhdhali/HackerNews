...
function Timeline() {
  ...
  const getTimeSinceUpdate = (updatedAt) => {
    ...
    const timeDiff = now - updatedTime; // time difference in milliseconds
        ...
        <ol className='articleList'>
          {articles.map((article) => (
            <li key={article.objectID} className='articleItem'>
              ...
              <p>
                {article.points} points | {article.num_comments} comments | {new Date(article.created_at).toLocaleString()} | {new Date(article.updated_at).toLocaleString()} | {getTimeSinceUpdate(article.updated_at)} hours ago
              </p>
              <p>
                posted by{' '}
